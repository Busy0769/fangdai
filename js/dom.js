var domOperate = {
    ...
    lilvCal: function (year, lilv, selectcontent, id) {
        ...
        lilv.forEach(function (item, index) {
            if (year >= item.year) {
                getLilv = item.lilv;//获取基准利率
            	//getLilv = 0.0465;
            }
        })
        var callilv = selectcontent.map(function (item, index) {
            return {
            	//var temp = item.lilv + getLilv;
    ...
    domOperates: function (type, data) {
        ...
        if ($("#infoDialog").length > 0) {
            ...
            //详情页面循环
            var yeartitle = '<div class="yeartitle">第一年 &nbsp&nbsp<a href="../" style="font-size: 16px;color: #FFF;">www.sexy0769.com</a></div>',
                yearHtmlArray = [],
                yeartitleHtml = "",
                mouthliArray = [],
                yearflag = 1;
